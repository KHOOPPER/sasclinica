...
export default async function SuperadminBillingPage() {
  ...
  return (
    <div className="space-y-6 md:space-y-12 pb-16">
      ...
      {/* Header */}
      <div>
        <h1 className="text-2xl md:text-3xl font-black text-text-main tracking-tight uppercase leading-none">Finanzas</h1>
        <p className="text-slate-400 text-[10px] md:text-sm font-medium mt-1.5 md:mt-2">
          Métricas clave y suscripciones
        </p>
      </div>
      ...
      {fetchError && (
        <div className="bg-rose-50 border-none rounded-2xl p-4 flex items-center gap-3 text-rose-600 shadow-sm">
          ...
          <p className="text-[13px] font-bold uppercase tracking-wide">Error al cargar datos financieros.</p>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-6">
        <div className="bg-card-bg rounded-2xl md:rounded-[2.5rem] p-4 md:p-8 shadow-card border border-slate-200/50 dark:border-white/5 flex flex-col justify-between group hover:-translate-y-1 transition-all">
          <div className="flex items-center justify-between mb-3 md:mb-4">
            <h3 className="text-[9px] md:text-[11px] font-black uppercase tracking-widest text-slate-400">MRR</h3>
            <div className="p-1.5 md:p-2.5 bg-emerald-50 text-emerald-600 rounded-lg md:rounded-xl">
              <DollarSign className="h-3.5 w-3.5 md:h-4 md:w-4" />
            </div>
          </div>
          <div>
            <p className="text-xl md:text-4xl font-black text-text-main tabular-nums">$0.00</p>
            <p className="text-[8px] md:text-xs font-medium text-emerald-500 mt-1">+0.00%</p>
          </div>
        </div>

        <div className="bg-card-bg rounded-2xl md:rounded-[2.5rem] p-4 md:p-8 shadow-card border border-slate-200/50 dark:border-white/5 flex flex-col justify-between group hover:-translate-y-1 transition-all">
          <div className="flex items-center justify-between mb-3 md:mb-4">
            <h3 className="text-[9px] md:text-[11px] font-black uppercase tracking-widest text-slate-400">Planes</h3>
          ...
          <div>
            <p className="text-xl md:text-4xl font-black text-text-main tabular-nums">{tenants.length}</p>
            <p className="text-[8px] md:text-xs font-medium text-slate-400 mt-1">Activos</p>
          </div>
        </div>
        ...
        <div className="col-span-2 md:col-span-1 bg-card-bg rounded-2xl md:rounded-[2.5rem] p-4 md:p-8 shadow-card border border-slate-200/50 dark:border-white/5 flex flex-row md:flex-col items-center md:items-start justify-between group hover:-translate-y-1 transition-all">
          <div className="flex items-center justify-between md:mb-4 w-full">
            <h3 className="text-[9px] md:text-[11px] font-black uppercase tracking-widest text-slate-400">Fallidos</h3>
            <div className="p-1.5 md:p-2.5 bg-rose-50 text-rose-600 rounded-lg md:rounded-xl">
          ...
          <div className="text-right md:text-left">
            ...
            <p className="text-[8px] md:text-xs font-medium text-slate-400 mt-0.5 md:mt-1">Atención</p>
          </div>
        </div>
      </div>
      ...
      <div>
        <h2 className="text-[11px] md:text-sm font-black uppercase tracking-widest text-slate-400 mb-4 md:mb-6 px-1 flex items-center gap-2 leading-none">
            ...
            <Table>
              <TableHeader>
                <TableRow className="hover:bg-transparent border-slate-100/50 dark:border-white/5 bg-slate-50/30 dark:bg-white/[0.02]">
                  ...
                  <TableHead className="py-4 md:py-5 px-4 md:px-6 text-[9px] md:text-[10px] font-black uppercase tracking-widest text-slate-400">Estado</TableHead>
                  <TableHead className="py-4 md:py-5 px-4 md:px-6 text-[9px] md:text-[10px] font-black uppercase tracking-widest text-slate-400">Próximo Cobro</TableHead>
                  <TableHead className="py-4 md:py-5 px-4 md:px-6 text-right text-[9px] md:text-[10px] font-black uppercase tracking-widest text-slate-400">Stripe</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {tenants.map((t: any) => (
                  <TableRow key={t.id} className="border-slate-100/50 dark:border-white/5 hover:bg-slate-50/50 dark:hover:bg-white/[0.01] transition-colors">
                    ...
                    <TableCell className="py-4 md:py-5 px-4 md:px-6">
                      <div className="flex items-center gap-3 md:gap-4 min-w-[150px]">
                        ...
                        <div>
                          <span className="block font-black text-text-main uppercase text-[11px] md:text-[13px] tracking-tight truncate max-w-[100px] md:max-w-none">{t.name}</span>
                          <span className="block font-medium text-slate-400 text-[9px] md:text-[11px] mt-0.5">/{t.slug}</span>
                        </div>
                      </div>
                    </TableCell>
                    ...
                    {/* Plan */}
                    <TableCell className="py-4 md:py-5 px-4 md:px-6">
                      <span className="inline-flex items-center text-[9px] md:text-[11px] font-black text-text-main uppercase tracking-widest whitespace-nowrap">
                        {t.plan || 'Básico'}
                      </span>
                    </TableCell>
                    ...
                    <TableCell className="py-4 md:py-5 px-4 md:px-6">
                      <div className="flex items-center gap-2 whitespace-nowrap">
                        ...
                        <span className="text-[9px] md:text-[11px] font-black text-text-main uppercase tracking-widest">Activo</span>
                      </div>
                    </TableCell>

                    {/* Next Bill */}
                    <TableCell className="py-4 md:py-5 px-4 md:px-6 text-[10px] md:text-[12px] text-slate-500 font-medium whitespace-nowrap">
                    ...
                    <TableCell className="py-4 md:py-5 px-4 md:px-6 text-right">
                      <div className="flex items-center justify-end">
                        <button title="Abrir Customer Portal (Stripe)" className="p-2 text-slate-300 hover:text-slate-600 transition-colors cursor-pointer outline-none active:scale-90">
