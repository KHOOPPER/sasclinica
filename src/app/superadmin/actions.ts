...
export async function updateWebsiteSettings(settings: any) {
  try {
    ...
    // Limpieza final de payload para evitar conflictos de ID
    delete payload.id
    payload.is_active = true // Siempre activo al guardar desde el editor
    ...
    // Purga específica por slug
    if (payload.slug) {
      const slugPath = `/${payload.slug}`
      revalidatePath(slugPath, 'page')
      revalidatePath(slugPath, 'layout')
      revalidatePath(slugPath)
    }

    // Revalidar también el root por si acaso
    revalidatePath('/')
...
export async function updateTenantPlan(tenantId: string, plan: string, startDate: string, expiryDate: string, mrr: number) {
  try {
    const supabase = await createClient()
    const { data: authData } = await supabase.auth.getUser()
    if (!authData.user) return { error: 'No autenticado' }
    ...
    const { error: tenantError } = await supabaseAdmin
      ...
      .update({ 
        plan, 
        // plan_starts_at: new Date(startDate + 'T00:00:00').toISOString(),
...
export async function createGlobalNotification(formData: FormData) {
  try {
    ...
    if (!title || !message) return { error: 'El título y el mensaje son obligatorios.' }
    ...
    if (!profile?.is_superadmin) return { error: 'Permisos insuficientes' }
    ...
    revalidatePath('/', 'layout') // Revalidate all layouts so headers get the notification
    return { success: true }
  } catch (error: any) {
    return { error: error.message || 'Error al enviar la notificación.' }
  }
}
