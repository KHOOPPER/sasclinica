...
export function PlanManagementModal({ isOpen, onClose, tenant }: PlanManagementModalProps) {
  ...
  const initialStartDate = tenant.plan_starts_at ? new Date(tenant.plan_starts_at).toISOString().split('T')[0] : new Date().toISOString().split('T')[0]
  ...
  const handleSave = async () => {
    ...
    const result = await updateTenantPlan(tenant.id, selectedPlan, startDate, expiryDate, planData?.price || 0)
    setLoading(false)
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Gestión de Plan SaaS">
      <div className="space-y-8 p-1">
        ...
        <div>
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4">Periodo de Servicio</p>
          <div className="flex flex-col gap-4 bg-slate-50 dark:bg-white/5 p-4 rounded-2xl border border-slate-100 dark:border-white/5">
            <div className="flex items-center justify-between">
              <div className="flex-1">
                <p className="text-xs font-bold text-text-main">Fecha de Inicio</p>
              </div>
            ...
            <div className="flex items-center justify-between">
              <div className="flex-1">
                <p className="text-xs font-bold text-text-main">Fecha de Vencimiento</p>
              </div>
              <div className="flex items-center gap-3">
                <input 
                  type="date"
        ...
        <div className="bg-slate-950 rounded-2xl md:rounded-[2rem] p-4 md:p-6 text-white overflow-hidden relative">
          <div className="relative z-10">
            ...
            <div className="flex items-baseline gap-2">
              <span className="text-2xl md:text-3xl font-black">{selectedPlan.toUpperCase()}</span>
              <span className="text-[9px] md:text-xs font-bold text-slate-400">Arquitectura Seleccionada</span>
            </div>
            <div className="mt-4 flex items-center gap-2 text-[9px] md:text-[10px] font-bold text-slate-400 uppercase">
