...
export default async function AdminPage() {
  ...
  return (
    <div className="space-y-6 md:space-y-10 max-w-[1600px] mx-auto pb-10 px-3.5 md:px-0">
      {/* Header Metric Cards */}
      {/* Header Metric Cards */}
      <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-6">
        {/* Revenue Card - Only for Admin/Receptionist */}
        {(isClinicAdmin || isReceptionist) && (
          <div className="bg-card-bg p-3.5 md:p-6 rounded-2xl md:rounded-3xl shadow-card border border-slate-200/50 dark:border-white/5 flex flex-col justify-between h-32 md:h-40 transition-all hover:-translate-y-1">
            <div className="flex items-center justify-between mb-1 md:mb-4">
              <div className="p-1.5 md:p-2 bg-slate-50/50 dark:bg-white/5 rounded-xl md:rounded-2xl">
                <DollarSign className="h-3.5 w-3.5 md:h-5 md:w-5 text-emerald-500" />
              </div>
            ...
            <div>
              <p className="text-[8px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest mb-0.5 md:mb-1">Ingresos (Mes)</p>
              <div className="flex items-baseline gap-2">
                <h3 className="text-lg md:text-2xl font-black text-text-main tracking-tight">${incomeMonth.toLocaleString()}</h3>
              </div>
            </div>
          </div>
        )}
        ...
        <div className="bg-card-bg p-3.5 md:p-6 rounded-2xl md:rounded-3xl shadow-card border border-slate-200/50 dark:border-white/5 flex flex-col justify-between h-32 md:h-40 transition-all hover:-translate-y-1">
          <div className="flex items-center justify-between mb-1 md:mb-4">
            <div className="p-1.5 md:p-2 bg-slate-50/50 dark:bg-white/5 rounded-xl md:rounded-2xl">
              <PlusCircle className="h-3.5 w-3.5 md:h-5 md:w-5 text-emerald-500" />
            </div>
            <div className="hidden sm:block">
          ...
          <div>
            <p className="text-[8px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest mb-0.5 md:mb-1">Pacientes Nuevos (Mes)</p>
            <div className="flex items-baseline gap-2">
              <h3 className="text-lg md:text-2xl font-black text-text-main tracking-tight">{newPatients}</h3>
            </div>
          </div>
        </div>
        ...
        <div className="bg-card-bg p-3.5 md:p-6 rounded-2xl md:rounded-3xl shadow-card border border-slate-200/50 dark:border-white/5 flex flex-col justify-between h-32 md:h-40 transition-all hover:-translate-y-1">
          <div className="flex items-center justify-between mb-1 md:mb-4">
            <div className="p-1.5 md:p-2 bg-slate-50/50 dark:bg-white/5 rounded-xl md:rounded-2xl">
              <CalendarCheck className="h-3.5 w-3.5 md:h-5 md:w-5 text-emerald-500" />
            </div>
            <div className="hidden sm:block">
          ...
          <div>
            ...
            <div className="flex items-baseline gap-2">
              <h3 className="text-lg md:text-2xl font-black text-text-main tracking-tight">{totalVisits}</h3>
            </div>
          </div>
        </div>
        ...
        <div className="bg-card-bg p-3.5 md:p-6 rounded-2xl md:rounded-3xl shadow-card border border-slate-200/50 dark:border-white/5 flex flex-col justify-between h-32 md:h-40 transition-all hover:-translate-y-1">
          <div className="flex items-center justify-between mb-1 md:mb-4">
            <div className="p-1.5 md:p-2 bg-slate-50/50 dark:bg-white/5 rounded-xl md:rounded-2xl">
              <XCircle className="h-3.5 w-3.5 md:h-5 md:w-5 text-slate-400" />
            </div>
            <div className="hidden sm:block">
          ...
          <div>
            <p className="text-[8px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest mb-0.5 md:mb-1">Cancelaciones (Mes)</p>
            <div className="flex items-baseline gap-2">
              <h3 className="text-lg md:text-2xl font-black text-text-main tracking-tight">{cancelledVisits}</h3>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8">
        {/* Recent Patients */}
        <div className="bg-card-bg rounded-[2rem] md:rounded-[2.5rem] shadow-card border border-slate-200/50 dark:border-white/5 overflow-hidden">
          <div className="px-6 md:px-8 py-5 md:py-6 border-b border-slate-100/50 dark:border-white/5 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="p-2 bg-slate-50/50 dark:bg-white/5 rounded-xl">
                <Activity className="h-3.5 w-3.5 md:h-4 md:w-4 text-emerald-500" />
              </div>
              <h3 className="text-sm md:text-md font-black text-text-main uppercase tracking-tight">Actividad Reciente</h3>
            </div>
            <Link href="/admin/pacientes" className="text-[9px] md:text-[10px] font-black text-slate-400 hover:text-emerald-500 uppercase tracking-widest transition-colors">Ver todos</Link>
          </div>
          
          <div className="p-2 md:p-0">
            {/* Mobile View: List */}
            <div className="md:hidden divide-y divide-slate-100/50 dark:divide-white/5">
              {recentPatients?.map((p) => (
                <div key={p.id} className="flex items-center justify-between p-4 group active:bg-slate-50 transition-all">
                  <div className="flex items-center gap-3">
                    <div className="h-9 w-9 rounded-full bg-slate-100 dark:bg-white/5 text-slate-500 flex items-center justify-center font-black text-[10px] border border-slate-200/50">
                    ...
                    <div>
                      ...
                      <p className="text-[9px] text-slate-400 font-bold mt-0.5">
                        Registrado: {new Date(p.created_at).toLocaleDateString()}
                      </p>
                    </div>
                  </div>
                  <span className="px-2 py-0.5 bg-emerald-500/10 text-emerald-600 text-[8px] font-black uppercase tracking-widest rounded-full border border-emerald-500/20">
                    Nuevo
                  </span>
                </div>
              ))}
              {(!recentPatients || recentPatients.length === 0) && (
                <div className="py-12 text-center text-slate-400 text-[10px] font-black uppercase italic tracking-widest">
            ...
            <div className="hidden md:block">
              <table className="w-full">
                ...
                <tbody className="divide-y divide-slate-50 dark:divide-white/5">
                  {recentPatients?.map((p) => (
                    <tr key={p.id} className="hover:bg-slate-50/30 dark:hover:bg-white/[0.01] transition-all group">
        ...
        <div className="bg-card-bg rounded-[2rem] md:rounded-[2.5rem] shadow-card border border-slate-200/50 dark:border-white/5 overflow-hidden">
          <div className="px-6 md:px-8 py-5 md:py-6 border-b border-slate-100/50 dark:border-white/5 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="p-2 bg-slate-50/50 dark:bg-white/5 rounded-xl">
                <Calendar className="h-3.5 w-3.5 md:h-4 md:w-4 text-emerald-500" />
              </div>
            <h3 className="text-sm md:text-md font-black text-text-main uppercase tracking-tight">Próximas Citas Hoy</h3>
            </div>
            <span className="text-[8px] md:text-[10px] font-black text-text-main bg-emerald-500/10 px-2 md:px-3 py-1 rounded-full border border-emerald-500/20 uppercase tracking-widest">
            ...
            {appointmentsToday?.slice(0, 5).map((a) => (
              <div key={a.id} className="flex items-center justify-between group p-2 hover:bg-slate-50/50 dark:hover:bg-white/5 rounded-2xl transition-all">
                <div className="flex items-center gap-3 md:gap-4">
                  <div className="h-10 md:h-12 w-1 md:w-1.5 bg-slate-100 dark:bg-white/10 group-hover:bg-emerald-500 transition-all rounded-full" />
                  <div>
                    <h4 className="text-xs md:text-sm font-black text-text-main group-hover:text-emerald-500 transition-colors uppercase tracking-tight truncate max-w-[150px] md:max-w-none">
                      {(a.patient as any)?.first_name} {(a.patient as any)?.last_name}
                    </h4>
                    <p className="text-[8px] md:text-[10px] text-slate-400 font-black uppercase tracking-widest truncate max-w-[120px] md:max-w-none">
                      {(a.service as any)?.name || 'Servicio General'}
                    </p>
                  </div>
                </div>
                <div className="text-right flex flex-col items-end gap-1">
                  <p className="text-xs md:text-sm font-black text-text-main tabular-nums">
                    {new Date(a.start_time).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                  </p>
                  <p className={`text-[7px] md:text-[8px] uppercase font-black tracking-[0.1em] px-1.5 py-0.5 rounded-md border ${(a.status === 'confirmed' || a.status === 'completed') ? 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20' : 'bg-amber-500/10 text-amber-500 border-amber-500/20'}`}>
