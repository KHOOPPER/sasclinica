...
export async function getClinicBySlug(slug: string) {
  ...
  // Somos extremadamente flexibles: buscamos por slug exacto o variaciones comunes
  const { data, error } = await supabase
    .from('public_clinic_settings')
    .select(`
      id, tenant_id, clinic_id, slug, is_active, 
      welcome_text, primary_color, secondary_color, accent_color,
      hero_title, hero_subtitle, hero_image_url, hero_badge, hero_cta_text,
      trust_badge_1, trust_badge_2, trust_badge_3,
      logo_url, logo_height, logo_width, logo_padding_top,
      footer_variant, header_variant, hero_variant,
      footer_text, footer_bg, navbar_bg, navbar_opacity, navbar_text_color,
      show_navbar_border, navbar_border_color, navbar_border_width,
      active_sections, font_headlines, font_body,
      show_whatsapp, whatsapp_number, whatsapp_message, whatsapp_position,
    ...
    `)
    .or(`slug.ilike.${cleanSlug},slug.ilike.${cleanSlug.replace(/-/g, '.')},slug.ilike.${cleanSlug.replace(/\./g, '-')}`)
    .order('updated_at', { ascending: false }) // Traer el más reciente si hay duplicados
    .maybeSingle()
  ...
  // 2. Si encontramos datos reales, LOS USAMOS 
  if (data) {
    return data
  }

  // 3. FALLBACK: Solo si NO existe ningún registro con ese slug (Auto-generación / Placeholder)
...
export async function createPublicAppointment(formData: any) {
  ...
  // 4. Crear notificación para la clínica
  await serviceClient.from('system_notifications').insert([{
    clinic_id: formData.clinic_id,
    title: 'Nueva Reserva Web',
    message: `El paciente ${formData.patient_name} ha agendado una cita para el ${format(new Date(formData.start_time), 'dd/MM/yyyy HH:mm')}.`,
    type: 'success' // Verde
  }])
