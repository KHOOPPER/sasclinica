...
import {
  ...
  TrendingUp, Database, ShieldAlert, ExternalLink
} from 'lucide-react'
import {
    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 pb-2">
        <div>
          ...
          <h1 className="text-3xl md:text-4xl font-black text-text-main tracking-tighter uppercase leading-none">
            Super Admin
          </h1>
          <p className="text-slate-400 text-xs md:text-sm font-medium mt-2">Centro de control de la red SaaS</p>
        </div>
        <div className="flex items-center gap-3">
          <div className="bg-card-bg border border-slate-200/50 dark:border-white/5 shadow-sm rounded-xl md:rounded-2xl px-4 md:px-5 py-2 md:py-3 text-right backdrop-blur-sm">
            <p className="text-[8px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">{mounted ? dateStr : '—'}</p>
            <p className="text-xl md:text-2xl font-black text-text-main tabular-nums tracking-tight leading-none">{mounted ? timeStr : '—:—'}</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        {/* Growth Chart */}
        <div className="xl:col-span-2 bg-card-bg rounded-2xl md:rounded-[2.5rem] shadow-card border border-slate-200/50 dark:border-white/5 overflow-hidden">
          <div className="p-5 md:p-8 pb-2 flex items-start justify-between">
            <div>
              <h2 className="text-lg md:text-xl font-black text-text-main uppercase tracking-tight flex items-center gap-2">
                <TrendingUp className="h-4 w-4 md:h-5 md:w-5 text-emerald-500" />
                Crecimiento
              </h2>
              <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-1">Tenants acumulados</p>
            </div>
            <Link href="/superadmin/clinicas" className="flex items-center gap-1 text-[9px] font-black text-emerald-500 uppercase tracking-widest hover:gap-2 transition-all">
              Ver <ArrowUpRight className="h-3 w-3" />
            </Link>
          </div>
          <div className="h-60 md:h-72 p-2 md:p-4 pb-6">
            {mounted && stats.growthData.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={stats.growthData} margin={{ top: 10, right: 0, left: -25, bottom: 0 }}>
                  <defs>
                  ...
                  <XAxis dataKey="name" axisLine={false} tickLine={false}
                    tick={{ fill: '#94a3b8', fontSize: 9, fontWeight: 700 }} dy={8} />
        ...
        {/* System Alerts */}
        <div className="bg-card-bg rounded-2xl md:rounded-[2.5rem] shadow-card border border-slate-200/50 dark:border-white/5 overflow-hidden flex flex-col">
          <div className="p-5 md:p-6 pb-4 flex items-center justify-between">
            <h2 className="text-sm md:text-base font-black text-text-main uppercase tracking-tight flex items-center gap-2">
          ...
          <div className="flex-1 px-4 pb-6 flex flex-col justify-center items-center text-center">
             <div className="h-12 w-12 md:h-16 md:w-16 rounded-2xl md:rounded-3xl bg-emerald-500/5 flex items-center justify-center mb-4 transition-colors">
               <CheckCircle2 className="h-6 w-6 md:h-8 md:w-8 text-emerald-500 opacity-80" />
             </div>
             <p className="text-xs md:text-sm font-black text-text-main uppercase tracking-tight leading-tight">Sistemas Operativos</p>
             <p className="text-[10px] md:text-[11px] text-slate-400 font-medium mt-1.5 max-w-[180px] mx-auto">No hay alertas críticas en las últimas 24h.</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        {/* TENANT TABLE & MOBILE CARDS */}
        <div className="xl:col-span-2 bg-card-bg rounded-[2rem] md:rounded-[2.5rem] shadow-card border border-slate-200/50 dark:border-white/5 overflow-hidden">
          <div className="p-6 md:p-8 flex items-center justify-between border-b border-slate-100/50 dark:border-white/5">
            <h2 className="text-sm md:text-base font-black text-text-main uppercase tracking-tight flex items-center gap-2">
              <Building2 className="h-4 w-4 text-emerald-500" />
              Red de Tenants
            </h2>
            <Link href="/superadmin/clinicas"
              className="text-[9px] md:text-[10px] font-black text-emerald-500 uppercase tracking-widest flex items-center gap-1 hover:gap-2 transition-all">
          ...
          <div className="block md:hidden p-4 space-y-4">
            {stats.tenantDetails.length === 0 ? (
              ...
              stats.tenantDetails.map((t: any) => (
                <div key={t.id} className="bg-slate-50/50 dark:bg-white/[0.02] border border-slate-100 dark:border-white/5 rounded-2xl p-4 space-y-4">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className="h-10 w-10 rounded-xl bg-brand-primary/10 flex items-center justify-center text-emerald-500 font-black text-sm uppercase">
                        {t.name[0]}
                      </div>
                      <div>
                        <p className="font-black text-text-main text-xs uppercase">{t.name}</p>
                        <code className="text-[10px] text-slate-400 font-mono">/{t.slug}</code>
                      </div>
                    </div>
                    <div className="text-right">
                      <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Registro</p>
                  ...
                  <div className="grid grid-cols-2 gap-4 py-2 border-y border-slate-100 dark:border-white/5">
                    <div className="text-center">
                      <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Clínicas</p>
                      <p className="text-xs font-black text-text-main">{Array.isArray(t.clinics) ? t.clinics.length : 0}</p>
                    </div>
                    <div className="text-center">
                      <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Usuarios</p>
                      <p className="text-xs font-black text-text-main">{Array.isArray(t.profiles) ? t.profiles.length : 0}</p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    {t.clinics && t.clinics.length > 0 && (
                      <Link 
                        ...
                        className="flex-1 inline-flex items-center justify-center gap-1.5 px-3 py-2.5 rounded-xl bg-slate-900 text-[10px] font-black text-white uppercase tracking-widest hover:bg-emerald-500 transition-colors shadow-sm"
                      >
                        Modo Dios
                      </Link>
                    )}
                    <Link href={`/${t.slug}`} target="_blank"
                      className="flex-1 inline-flex items-center justify-center gap-2 px-3 py-2.5 rounded-xl border border-slate-200 dark:border-white/10 text-[10px] font-black text-slate-500 uppercase tracking-widest hover:bg-slate-50 transition-colors">
...
function SaaSKpiCard({ label, value, subtitle, subtitleColor, icon: Icon, color, href }: {
  ...
  const inner = (
    <div className="bg-card-bg rounded-2xl md:rounded-[2.5rem] shadow-card border border-slate-200/50 dark:border-white/5 p-5 md:p-8 dark:hover:shadow-emerald-500/5 hover:-translate-y-1 transition-all group cursor-pointer flex flex-col justify-between min-h-[130px] md:min-h-[160px]">
      <div className="flex items-start gap-3 md:gap-4 mb-2 md:mb-3">
        <div className={`h-9 w-9 md:h-12 md:w-12 rounded-xl md:rounded-2xl ${c.bg} flex items-center justify-center shrink-0 group-hover:scale-105 transition-transform`}>
          <Icon className={`h-5 w-5 md:h-6 md:w-6 ${c.text}`} />
        </div>
        <p className="text-[9px] md:text-[11px] font-black uppercase tracking-[0.1em] text-slate-500 leading-snug pt-1">{label}</p>
      </div>
      <div>
        <p className="text-2xl md:text-4xl font-black text-text-main tracking-tighter tabular-nums leading-none mb-1 md:mb-1.5">{value}</p>
        <p className={`text-[9px] md:text-[11px] font-bold ${subtitleColor} tracking-wide truncate`}>{subtitle}</p>
      </div>
    </div>
  )
